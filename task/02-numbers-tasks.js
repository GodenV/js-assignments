...
function getAngleBetweenVectors(x1, y1, x2, y2) {
    return Math.acos((x1*x2+y1*y2)/(Math.pow(x1*x1+y1*y1, 0.5)*Math.pow(y2*y2+x2*x2, 0.5)));
}
...
function roundToPowerOfTen(num, pow) {
        return num-(num%(Math.pow(10,pow))>Math.pow(10,pow)/2 ? -(Math.pow(10,pow)-num%Math.pow(10,pow)):num%(Math.pow(10,pow)));
}
...
function toNumber(value, def) {
     return value > def ? Number(value) : Number(def);
}
